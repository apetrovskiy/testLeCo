class Solution:
    def romanToInt(self, s: str) -> int:
        self.result = 0
        s = self.calculate_part(s, one="I", half="V", next_digit="X", multiplier=1)
        s = self.calculate_part(s, one="X", half="L", next_digit="C", multiplier=10)
        s = self.calculate_part(s, one="C", half="D", next_digit="M", multiplier=100)
        if "MMM" in s:
            self.result += 3000
        elif "MM" in s:
            self.result += 2000
    ...
    def calculate_part(
        self, s: str, one: str, half: str, next_digit: str, multiplier: int
    ) -> str:
        if s is None or s == "":
            return ""
